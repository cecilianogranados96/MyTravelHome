...
if (isset($_GET['borr'])){
    $query = "DELETE FROM tipo_habitacion WHERE id_tipo_habitacion = '".$_GET['borr']."'";
    $result = mysql_query($query) or die('Consulta fallida: ' . mysql_error());
    echo '<script>alert("¡Borrado con éxito!"); window.location.href = "?pag='.$_GET['pag'].'&pagina=habitaciones";</script>';
}

if (isset($_GET['edit'])){

        $query = "SELECT nombre FROM `tipo_habitacion` WHERE id_tipo_habitacion= '".$_GET['edit']."' ";
        $result = mysql_query($query) or die('Consulta fallida: ' . mysql_error());
        $datos = mysql_fetch_array($result, MYSQL_ASSOC);    
}    

if (isset($_GET['insert'])){        
    $query = "INSERT INTO `tipo_habitacion`(`id_tipo_habitacion`, `nombre`) VALUES ('".mysql_insert_id()."','".$_POST['nombre_n']."')";
    $result = mysql_query($query) or die('Consulta fallida: ' . mysql_error());
    echo '<script>alert("¡Registrado con éxito!"); window.location.href = "?pag='.$_GET['pag'].'&pagina=habitaciones";</script>';
}
...
<div class="db-cent-table db-com-table">
    <div class="db-title">
        ...
        <br>
        <center>
            <h3><img src="images/icon/dbc4.png" />Catálogo de tipo de habitaciones</h3>
        </center>
    </div>
    <!--*************************EDITAR*************************-->
            ...
            <?php 
                ...
                while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {  
                    echo "    <tr>
                                ...
                                <td>
                                    <center><a href='?pag=".$_GET['pag']."&pagina=".$_GET['pagina']."&edit=".$line['id_tipo_habitacion']."' class='btn btn-success'>Editar</a></center>
                                </td>
                                <td>
                                    <center><a href='?pag=".$_GET['pag']."&pagina=".$_GET['pagina']."&borr=".$line['id_tipo_habitacion']."' class='btn btn-danger'>Borrar</a></center>
                                </td>
                            </tr>
                ";
                }                   
